 ...
 * 
 * 
 //ID DE DISPOSITIVOS AGREGADOS A LA RED: colocar el id en formato hexagesimal en el siguiente array
 */
 static const uint32_t id_local_device_registre[] ={0X830fab,0x008313c3, 0x008313ab,0x008313c4,0x007FF3EF, 0x3faecb,0x003faf4f, 0x00449671, 0x008311D1,0x8300cf, 0x00830951,0x00830d6c,0x00830d6c,0x00830505, 0x00830951 }; 
...
void setup() {
  ...
  delay(10000);
  SerialAT.println("AT+CGATT=1");  delay(200);
  modem.init();
  digitalWrite(RXLED,LOW);
...
void loop() {
  ...
  if (!modem.isNetworkConnected()) {
    SerialMon.println(F("NTW DISCON"));
    ...
    if (modem.isNetworkConnected()) {
      SerialMon.println(F("NTW RECONN"));
    }

    // and make sure GPRS/EPS is still connected
    if (!modem.isGprsConnected()) {
      SerialMon.println(F("GPRS DISCON"));
      SerialMon.print(F("CONN to "));
      SerialMon.print(apn);
      if (!modem.gprsConnect(apn, gprsUser, gprsPass)) {
        SerialMon.println(" fail");
        delay(10000);
      ...
      if (modem.isGprsConnected()) { SerialMon.println("GPRS RECONN"); }
    }
  }
